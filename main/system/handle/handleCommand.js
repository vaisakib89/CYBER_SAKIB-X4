    ...
    return async function ({ event }) {
        ...
        if (commandName === `${PREFIX}boton` || commandName === `${PREFIX}botoff`) {
            ...
            if (!ADMINBOT.includes(senderID) && !OWNER.includes(senderID) && !OPERATOR.includes(senderID)) {
                return api.sendMessage("শুধুমাত্র বট অ্যাডমিন বা মালিক এই কমান্ড ব্যবহার করতে পারেন।", threadID, messageID);
            }
        ...
        if (typeof body === "string" && body.startsWith(`${PREFIX}request`) && approval) {
            if (APPROVED.includes(threadID)) {
                return api.sendMessage('এই বক্সটি ইতিমধ্যে অনুমোদিত', threadID, messageID);
            }
            let ryukodev;
            let request;
            var groupname = await global.data.threadInfo.get(threadID).threadName || "নাম পাওয়া যায়নি";
            ryukodev = `গ্রুপের নাম: ${groupname}\nগ্রুপ আইডি: ${threadID}`;
            request = `${groupname} গ্রুপ অনুমোদনের জন্য অনুরোধ করছে`;
            try {
                send('বক্স অনুমোদনের অনুরোধ', request + '\n\n' + ryukodev);
                api.sendMessage('আপনার অনুরোধ বট অপারেটরদের কাছে মেইলের মাধ্যমে পাঠানো হয়েছে।', threadID, messageID);
            } catch (error) {
                logger.err(error);
            }
        }
        ...
        if (userBanned.has(senderID) || threadBanned.has(threadID) || allowInbox == false && senderID == threadID) {
            if (!ADMINBOT.includes(senderID.toString()) && !OWNER.includes(senderID.toString()) && !OPERATOR.includes(senderID.toString())) {
                if (command && (command.config.name.toLowerCase() === commandName.toLowerCase()) && userBanned.has(senderID)) {
                    ...
                    return api.sendMessage(`আপনি বট ব্যবহার করতে পারবেন না\nকারণ: ${reason}\nব্যানের তারিখ: ${dateAdded}`, threadID, async (err, info) => {
                        await new Promise(resolve => setTimeout(resolve, 5 * 1000));
                        return api.unsendMessage(info.messageID);
                    }, messageID);
                } else {
        ...
        if (premium) {
            if (command && command.config) {
                if (command.config.premium && !premiumlists.includes(senderID)) {
                    return api.sendMessage(`এই কমান্ডটি শুধুমাত্র প্রিমিয়াম ব্যবহারকারীদের জন্য। প্রিমিয়াম ব্যবহার করতে অ্যাডমিন বা অপারেটরদের সাথে যোগাযোগ করুন অথবা ${PREFIX}requestpremium টাইপ করুন।`, event.threadID, async (err, eventt) => {
                        if (err) {
                            return;
                        }
                        await new Promise(resolve => setTimeout(resolve, 5 * 1000));
